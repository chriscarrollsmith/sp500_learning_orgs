...
- **The headline signal is largely a sector allocation effect.** Technology companies score highest on learning-org language (mean composite 0.145) and also returned the most over the 2005-2025 sample (8.2% per 6 months). Once composite scores are z-scored within sector and year, the correlation with forward returns falls from r = +0.031 to r = +0.005 — nearly zero. Within-sector quintile spreads are mixed: Energy and Financial Services show positive within-sector spreads, while Consumer Cyclical, Healthcare, and Communication Services show negative ones. The learning-org score should not be used as a stock-picking signal without controlling for sector exposure.
